...
public class UsuarioJpaController implements Serializable {
    ...
    public Usuario getUsuarioPorNombre(String nombre){
        EntityManager em=getEntityManager();
        try{
            List<Usuario> user = findUsuarioEntities();
            for(int i=0; i<user.size(); i++){
                if(user.get(i).getNombre().equals(nombre))
                    return user.get(i);
            }
        }finally{
            em.close();
    ...
    public List<Usuario> getUsuarioDesc(){
        EntityManager em=getEntityManager();
        try{
            Query q=em.createNativeQuery("SELECT * FROM USUARIO ORDER BY RACHA DESC",Usuario.class);
            return q.getResultList();
        }finally{
            em.close();
        }
